...
export const agentTools = {
    ...
    read_email_thread: tool({
        description:
            'Trae el TEXTO del hilo de uno de los correos de list_inbox_items, para resumirlo o citarlo cuando el usuario pregunte "¿de qué trata ese correo?" o "resúmeme el hilo de...". SIEMPRE llama primero a list_inbox_items para conseguir el inbox_item_id ACTUAL de ese hilo, aunque el usuario ya lo haya mencionado antes en esta misma conversación: cada resincronización puede cambiarlo, y un id viejo puede apuntar a un hilo distinto o a nada. Si esta herramienta devuelve ok:false (ej. "Ese correo no está en la lista"), NO te respaldes en algo que hayas dicho antes en la conversación (sería una versión vieja) — dile al usuario que ese correo ya no está sincronizado con ese id, vuelve a llamar list_inbox_items para buscarlo por asunto/remitente, y si no aparece, dilo así de claro. Puede no haber cuerpo disponible aun encontrando el id correcto: correos que el puente clasificó como ruido automático nunca lo traen, y el de cualquier correo vence a los 30 días de sincronizado (se borra solo). Si body es null, dilo tal cual ("no tengo el cuerpo de ese, solo el asunto") en vez de inventar contenido. Las contraseñas u otras claves que pudiera traer el correo ya vienen tachadas desde el origen (verás "[omitido]"); si el usuario necesita ese dato, dile que lo busque en Spark directamente.',
        inputSchema: z.object({
